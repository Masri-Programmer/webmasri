<template>
    <div class="fixed top-0 right-0 left-0 z-50">
        <div class="bg-secondary text-xs text-secondary-foreground shadow-sm">
            <div
                class="container mx-auto flex h-8 max-w-7xl cursor-pointer items-center justify-center gap-6 px-4 py-0 text-primary transition-all duration-100 ease-in-out hover:font-bold sm:px-6 lg:px-8"
            >
                    ...
                    <Sheet v-model:open="isSheetOpen">
                        ...
                        <SheetContent side="right" class="w-[300px] bg-sidebar text-sidebar-foreground sm:w-[400px]">
                            ...
                            <div class="flex flex-col gap-1.5 space-y-3 p-4 text-left">
                                ...
                                <a :href="contact.url()" @click="isSheetOpen = false" class="font-medium hover:underline">{{
                                    t('navigation.contact')
                                }}</a>
                                <a href="https://masri.blog/Projects/Main" @click="isSheetOpen = false" class="font-medium hover:underline">{{
                                    t('navigation.home.projects')
                                }}</a>
                                <Link :href="pricing.url()" @click="isSheetOpen = false" class="font-medium hover:underline">{{
                                    t('navigation.home.pricing')
                                }}</Link>
                                <Link :href="faq.url()" @click="isSheetOpen = false" class="font-medium hover:underline">{{
                                    t('navigation.home.faq')
                                }}</Link>
