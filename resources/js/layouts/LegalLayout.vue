...
const isStandardSectionPage = computed(() => ['privacyPolicy', 'termsConditions'].includes(props.pageKey));
</script>
                    ...
                    <section v-for="(section, key) in sections" :key="key" class="space-y-4">
                        ...
                        <div class="mt-4 space-y-4">
                            <template v-for="(item, index) in section.content" :key="index">
                                <p v-if="item.type === 'p'" :class="{ 'font-semibold': item.style === 'bold' }" v-html="item.text" />
